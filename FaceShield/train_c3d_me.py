...
def run_training(train_list, valid_list, k_num):
    ...
    model_filename = "model222/c3d_me_model-5000.model"
    ...
    with tf.Graph().as_default():
        ...
        for step in xrange(1,FLAGS.max_steps):
            ...
            if(step % 10 ==0 ):
                ...
                if (step) % 1000 == 0 or (step + 1) == FLAGS.max_steps or acc > 0.7:
                    saver.save(sess, os.path.join(model_save_dir, 'c3d_me_model'+str(k_num)), global_step=step)
                
    print("done")

def main(_):
    #k_num = 3
    for k_num in range(5):
        run_training('list/train_part_'+str(k_num)+'.list', 'list/valid_part_'+str(k_num)+'.list',k_num)
